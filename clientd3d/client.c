...
#include "client.h"
// #include "cmdpipe.h"  // Temporarily disabled
                   ...
                   _In_ LPSTR lpszCmdParam, _In_ int nCmdShow)
{
	...
	// Initialize command pipe for external admin tools
	// CommandPipeInit();  // Temporarily disabled
...
/* attempt make a crc16 on the meridian.exe */
	...
	while (!bQuit)
	{
		...
		// Process pending commands from external admin tools
		// CommandPipePoll();  // Temporarily disabled
	}

	/* Unregister our custom classes--not good to leave them around */
	UnregisterWindowClasses();

	// Shutdown command pipe
	// CommandPipeClose();  // Temporarily disabled
