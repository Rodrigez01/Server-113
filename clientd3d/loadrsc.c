...
static char *ConvertUtf8ToAcpIfNeeded(const char *src);
...
/******************************************************************************/
/*
* ConvertUtf8ToAcpIfNeeded: Convert UTF-8 text to system ANSI if possible.
* Returns newly allocated converted string, or NULL to keep original bytes.
*/
static char *ConvertUtf8ToAcpIfNeeded(const char *src)
{
   int i;
   ...
   // ASCII needs no conversion.
   if (!has_non_ascii)
      return NULL;

   // Only convert if the input is valid UTF-8.
   wide_len = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, src, -1, NULL, 0);
   if (wide_len <= 0)
      return NULL;

   wide = (WCHAR *) SafeMalloc(wide_len * sizeof(WCHAR));
   if (MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, src, -1, wide, wide_len) <= 0)
   {
      SafeFree(wide);
      return NULL;
   }

   acp_len = WideCharToMultiByte(CP_ACP, WC_NO_BEST_FIT_CHARS, wide, -1, NULL, 0, NULL, &used_default);
   if (acp_len <= 0 || used_default)
   {
      SafeFree(wide);
      return NULL;
   }

   converted = (char *) SafeMalloc(acp_len);
   if (WideCharToMultiByte(CP_ACP, WC_NO_BEST_FIT_CHARS, wide, -1, converted, acp_len, NULL, &used_default) <= 0
      || used_default)
   {
      SafeFree(converted);
      SafeFree(wide);
...
bool RscAddCallback(char *fname, int res, int lang_id, char *string)
{
   resource_type r;
   char *converted = NULL;
   const char *final_string = string;

   converted = ConvertUtf8ToAcpIfNeeded(string);
   if (converted != NULL)
      final_string = converted;
   ...
   r->resource[lang_id] = (char *) SafeMalloc(strlen(final_string) + 1);
   strcpy(r->resource[lang_id], final_string);
