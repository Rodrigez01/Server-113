...
$srcPng = Join-Path $PSScriptRoot "seelenfragment.png"
$workBmp = Join-Path $PSScriptRoot "isoulfrag.bmp"
$palFile = Join-Path $repo "blakston.pal"
$makebgf = Join-Path $repo "bin\\makebgf.exe"
$bgfName = "isoulfrag.bgf"
...
function Convert-ToM59IconBmp {
    ...
    # Infer a solid background color from the top-left corner and key it out.
    $bg = $src.GetPixel(0, 0)
    ...
    for ($y = 0; $y -lt $src.Height; $y++) {
        for ($x = 0; $x -lt $src.Width; $x++) {
            $p = $src.GetPixel($x, $y)
            $isTransparent = Is-BackgroundPixel -px $p -bgColor $bg -tol $bgTolerance
    ...
    for ($y = 0; $y -lt $size; $y++) {
        for ($x = 0; $x -lt $size; $x++) {
            ...
            if (Is-BackgroundPixel -px $p -bgColor $bg -tol $bgTolerance `
                -or $isNearBlack `
                -or ($p.G -ge 245 -and $p.B -ge 245 -and $p.R -le 10)) {
    ...
    # Force transparent background: flood-fill from edges using the corner index.
    $cornerIdx = @(
        [int]$raw[0],
        [int]$raw[$size - 1],
    ...
    function Enqueue-IfBg {
        ...
        $visited[$id] = $true
        if ([int]$raw[$yy * $stride + $xx] -eq $bgIndex) {
            $q.Enqueue($id)
        }
    }
...
& $makebgf -o $bgfOut -n "soul fragment" 1 $workBmp 1 1 1
Pop-Location
...
foreach ($d in $destDirs) {
    ...
    if ($dst -ne $bgfOut) {
        Copy-Item -Path $bgfOut -Destination $dst -Force
    }
}
