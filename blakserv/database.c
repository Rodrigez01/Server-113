...
char* password			= 0;
char* db				= 0;
static CRITICAL_SECTION mysql_mutex;
static LONG mysql_mutex_ready = 0;

static void MySQLInitMutex(void)
{
	if (InterlockedCompareExchange(&mysql_mutex_ready, 1, 0) == 0)
		InitializeCriticalSection(&mysql_mutex);
}

static void MySQLDestroyMutex(void)
{
	if (InterlockedCompareExchange(&mysql_mutex_ready, 0, 1) == 1)
		DeleteCriticalSection(&mysql_mutex);
}

void MySQLLock(void)
{
	if (mysql_mutex_ready)
		EnterCriticalSection(&mysql_mutex);
}

void MySQLUnlock(void)
{
	if (mysql_mutex_ready)
		LeaveCriticalSection(&mysql_mutex);
}
...
{
	return (mysql != NULL && state >= SCHEMAVERIFIED);
}
...
void _MySQLVerifySchema()
{
   ...
   mysql_query(mysql, SQLQUERY_CREATETABLE_GUILD);
	...
	// Create account tables and procedures
	_MySQLCreateAccountSchema();

	// Create quest tables and procedures
	_MySQLCreateQuestSchema();

	// Create item tables
	_MySQLCreateItemSchema();

	// Create monster tables
	_MySQLCreateMonsterSchema();

	// Create NPC tables
	_MySQLCreateNPCSchema();

	// Create Spell tables
	_MySQLCreateSpellSchema();

	// Create Skill tables
	_MySQLCreateSkillSchema();

	// Create Room tables
	_MySQLCreateRoomSchema();
   ...
   mysql_query(mysql, SQLQUERY_CREATEPROC_GUILDDISBAND);
	...
	// Load templates from database (export happens later via MySQLAutoExport)
	LoadQuestTemplatesFromMySQL();
	LoadItemTemplatesFromMySQL();
	LoadMonsterTemplatesFromMySQL();
	LoadNPCTemplatesFromMySQL();
	LoadSpellTemplatesFromMySQL();
	LoadSkillTemplatesFromMySQL();
	LoadRoomTemplatesFromMySQL();
	LoadPlayerTemplatesFromMySQL();

	MySQLUnlock();
};

/**
 * @brief Auto-export KOD data to MySQL if enabled
 *
 * This function should be called AFTER LoadKodbase() completes,
 * so that KOD classes are available for export.
 *
 * Behavior controlled by [MySQL] AutoExport config:
 * - AutoExport Yes: Always export (UPSERT - updates existing, adds new)
 * - AutoExport No:  Never auto-export
 *
 * @param force_export If true, export regardless of AutoExport setting
 */
void MySQLAutoExport(int force_export)
{
	...
	// Export quest data
	{
		int exported = MySQLExportQuestData();
		if (exported > 0)
		{
			// Reload after export to update cache
			MySQLLock();
			LoadQuestTemplatesFromMySQL();
			MySQLUnlock();
		}
	...
	// Export item data
	{
		int exported = MySQLExportItemData();
		if (exported > 0)
		{
			// Reload after export to update cache
			MySQLLock();
