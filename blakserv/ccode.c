...
/* ==================== QUEST LOADER C FUNCTIONS ==================== */

/**
 * @brief KOD-aufrufbare Funktion: Lädt Quest-Template-Count
 *
 * Gibt die Anzahl der geladenen Quest-Templates zurück.
 *
 * @return Anzahl der Templates als Integer
 */
int C_GetQuestTemplateCount(int object_id, local_var_type *local_vars,
                            int num_normal_parms, parm_node normal_parm_array[],
                            int num_name_parms, parm_node name_parm_array[])
{
   val_type ret_val;
   ...
   GetAllQuestTemplates(&count);

   ret_val.v.tag = TAG_INT;
   ret_val.v.data = count;

   return ret_val.int_val;
}

/**
 * @brief KOD-aufrufbare Funktion: Lädt Quest-Template-Daten
 *
 * Parameter: #quest_id (Integer) - Quest Template ID
 *
 * @return Liste mit Quest-Template-Daten oder NIL wenn nicht gefunden
 *         Format: [quest_name, quest_type, min_level, max_level, repeatable, active]
 */
int C_GetQuestTemplateData(int object_id, local_var_type *local_vars,
                           int num_normal_parms, parm_node normal_parm_array[],
                           int num_name_parms, parm_node name_parm_array[])
{
   val_type quest_id_val, ret_val;
   int quest_id;
   quest_template_t* qt;
   ...
   {
      bprintf("C_GetQuestTemplateData requires exactly 1 parameter (quest_id)\n");
      return ret_val.int_val;
   }

   quest_id_val = RetrieveValue(object_id, local_vars, normal_parm_array[0].type,
                                 normal_parm_array[0].value);

   if (quest_id_val.v.tag != TAG_INT)
   {
      bprintf("C_GetQuestTemplateData: quest_id must be an integer\n");
      return ret_val.int_val;
   }

   quest_id = quest_id_val.v.data;
   qt = GetQuestTemplateByID(quest_id);

   if (qt == NULL)
   {
      // Quest not found, return NIL
      return ret_val.int_val;
   }

   // Build return list: [name, type, min_level, max_level, repeatable, active]
   val_type quest_name, quest_type, min_level, max_level, repeatable, active;
   val_type nil_val, list_val;

   nil_val.v.tag = TAG_NIL;
   nil_val.v.data = NIL;

   quest_name.v.tag = TAG_RESOURCE;
   quest_name.v.data = AddDynamicResource(qt->quest_name);

   quest_type.v.tag = TAG_RESOURCE;
   quest_type.v.data = AddDynamicResource(qt->quest_type);

   min_level.v.tag = TAG_INT;
   min_level.v.data = qt->min_level;

   max_level.v.tag = TAG_INT;
   max_level.v.data = qt->max_level;

   repeatable.v.tag = TAG_INT;
   repeatable.v.data = qt->repeatable;

   active.v.tag = TAG_INT;
   active.v.data = qt->active;

   // Create list node chain using Cons from innermost to outermost
   list_val.v.tag = TAG_LIST;
   list_val.v.data = Cons(active, nil_val);

   list_val.v.tag = TAG_LIST;
   list_val.v.data = Cons(repeatable, list_val);

   list_val.v.tag = TAG_LIST;
   list_val.v.data = Cons(max_level, list_val);

   list_val.v.tag = TAG_LIST;
   list_val.v.data = Cons(min_level, list_val);

   list_val.v.tag = TAG_LIST;
   list_val.v.data = Cons(quest_type, list_val);

   ret_val.v.tag = TAG_LIST;
   ret_val.v.data = Cons(quest_name, list_val);

   return ret_val.int_val;
}

/**
 * @brief KOD-aufrufbare Funktion: Prüft Quest-Verfügbarkeit für Spieler
 *
 * Parameter: #quest_id (Integer), #player_level (Integer)
 *
 * @return TRUE wenn verfügbar, FALSE sonst
 */
int C_IsQuestAvailable(int object_id, local_var_type *local_vars,
                       int num_normal_parms, parm_node normal_parm_array[],
                       int num_name_parms, parm_node name_parm_array[])
{
   val_type quest_id_val, player_level_val, ret_val;
   int quest_id, player_level;
   ...
   if (num_normal_parms != 2)
   {
      bprintf("C_IsQuestAvailable requires 2 parameters (quest_id, player_level)\n");
      return ret_val.int_val;
   }

   quest_id_val = RetrieveValue(object_id, local_vars, normal_parm_array[0].type,
                                 normal_parm_array[0].value);
   player_level_val = RetrieveValue(object_id, local_vars, normal_parm_array[1].type,
                                    normal_parm_array[1].value);

   if (quest_id_val.v.tag != TAG_INT || player_level_val.v.tag != TAG_INT)
   {
      bprintf("C_IsQuestAvailable: parameters must be integers\n");
      return ret_val.int_val;
   }

   quest_id = quest_id_val.v.data;
   player_level = player_level_val.v.data;

   ret_val.v.data = IsQuestAvailableForPlayer(quest_id, player_level) ? True : False;

   return ret_val.int_val;
}

/* ============================================================================
 * DEPRECATED QUEST NODE FUNCTIONS
 *
 * These functions were part of questnodeloader.c which has been removed.
 * Quest triggers are now handled by the in-memory cache in questloader.c
 * See: CheckQuestTrigger(), LoadQuestTriggersFromMySQL()
 * ============================================================================
 */

/**
 * C_GetQuestNodeData: DEPRECATED - Returns NIL
 */
int C_GetQuestNodeData(int object_id, local_var_type *local_vars,
                       int num_normal_parms, parm_node normal_parm_array[],
                       int num_name_parms, parm_node name_parm_array[])
{
   val_type ret_val;
...
/**
 * C_GetQuestNodeNPCs: DEPRECATED - Returns NIL
 */
int C_GetQuestNodeNPCs(int object_id, local_var_type *local_vars,
                       int num_normal_parms, parm_node normal_parm_array[],
                       int num_name_parms, parm_node name_parm_array[])
{
   val_type ret_val;
...
/**
 * C_GetQuestNodeDialog: DEPRECATED - Returns NIL
 */
int C_GetQuestNodeDialog(int object_id, local_var_type *local_vars,
                         int num_normal_parms, parm_node normal_parm_array[],
                         int num_name_parms, parm_node name_parm_array[])
{
   val_type ret_val;
...
/**
 * C_GetQuestNodeRewards: DEPRECATED - Returns NIL
 */
int C_GetQuestNodeRewards(int object_id, local_var_type *local_vars,
                          int num_normal_parms, parm_node normal_parm_array[],
                          int num_name_parms, parm_node name_parm_array[])
{
   val_type ret_val;
 ...
 *
 * These functions provide access to item templates loaded from MySQL.
 * See: itemloader.c for loading implementation
 * ============================================================================
 */

/**
 * @brief KOD callable function: Returns number of loaded item templates
 *
 * @return Integer count of loaded templates
 */
int C_GetItemTemplateCount(int object_id, local_var_type *local_vars,
                           int num_normal_parms, parm_node normal_parm_array[],
                           int num_name_parms, parm_node name_parm_array[])
{
   val_type ret_val;
...
/**
 * @brief KOD callable function: Gets item template data by ID
 *
 * Parameter: #item_id (Integer) - Item Template ID
 *
 * @return List with item template data or NIL if not found
 *         Format: [item_name, item_kod_class, weight, bulk, value_average,
 *                  hits_min, hits_max, use_type, item_type, category]
 */
int C_GetItemTemplateData(int object_id, local_var_type *local_vars,
                          int num_normal_parms, parm_node normal_parm_array[],
                          int num_name_parms, parm_node name_parm_array[])
{
   val_type item_id_val, ret_val;
   int item_id;
   item_template_t* it;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_GetItemTemplateData requires exactly 1 parameter (item_id)\n");
      return ret_val.int_val;
   }

   item_id_val = RetrieveValue(object_id, local_vars, normal_parm_array[0].type,
                                normal_parm_array[0].value);

   if (item_id_val.v.tag != TAG_INT)
   {
      bprintf("C_GetItemTemplateData: item_id must be an integer\n");
      return ret_val.int_val;
   }

   item_id = item_id_val.v.data;
   it = GetItemTemplateByID(item_id);
   ...
   // Build return list: [name, kod_class, weight, bulk, value, hits_min, hits_max, use_type, item_type, category]
   val_type item_name, item_class, weight, bulk, value, hits_min, hits_max;
   val_type use_type, item_type, category;
   val_type nil_val, list_val;

   nil_val.v.tag = TAG_NIL;
   nil_val.v.data = NIL;

   item_name.v.tag = TAG_RESOURCE;
   item_name.v.data = AddDynamicResource(it->item_name);

   item_class.v.tag = TAG_RESOURCE;
   item_class.v.data = AddDynamicResource(it->item_kod_class);

   weight.v.tag = TAG_INT;
   weight.v.data = it->weight;

   bulk.v.tag = TAG_INT;
   bulk.v.data = it->bulk;

   value.v.tag = TAG_INT;
   value.v.data = it->value_average;

   hits_min.v.tag = TAG_INT;
   hits_min.v.data = it->hits_init_min;

   hits_max.v.tag = TAG_INT;
   hits_max.v.data = it->hits_init_max;

   use_type.v.tag = TAG_INT;
   use_type.v.data = it->use_type;

   item_type.v.tag = TAG_INT;
   item_type.v.data = it->item_type;

   category.v.tag = TAG_INT;
   category.v.data = (int)it->item_category;

   // Build list from back to front using Cons
   list_val.int_val = Cons(category, nil_val);
   list_val.int_val = Cons(item_type, list_val);
   list_val.int_val = Cons(use_type, list_val);
   list_val.int_val = Cons(hits_max, list_val);
   list_val.int_val = Cons(hits_min, list_val);
   list_val.int_val = Cons(value, list_val);
   list_val.int_val = Cons(bulk, list_val);
   list_val.int_val = Cons(weight, list_val);
   list_val.int_val = Cons(item_class, list_val);
   list_val.int_val = Cons(item_name, list_val);

   return list_val.int_val;
}

/**
 * @brief KOD callable function: Gets item template by KOD class name
 *
 * Parameter: #class_name (String/Resource) - KOD class name
 *
 * @return Item template ID or 0 if not found
 */
int C_GetItemTemplateByClass(int object_id, local_var_type *local_vars,
                             int num_normal_parms, parm_node normal_parm_array[],
                             int num_name_parms, parm_node name_parm_array[])
{
   val_type class_val, ret_val;
   item_template_t* it;
   char class_name[256];
   int len;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_GetItemTemplateByClass requires exactly 1 parameter (class_name)\n");
      return ret_val.int_val;
   }
   ...
   if (class_val.v.tag == TAG_RESOURCE)
   {
      resource_node* r = GetResourceByID(class_val.v.data);
      if (r != NULL && r->resource_val[0] != NULL)
      {
         strncpy(class_name, (const char*)r->resource_val[0], sizeof(class_name) - 1);
   ...
   else if (class_val.v.tag == TAG_STRING)
   {
      string_node* s = GetStringByID(class_val.v.data);
      if (s)
      {
         len = s->len_data < sizeof(class_name) - 1 ? s->len_data : sizeof(class_name) - 1;
         memcpy(class_name, s->data, len);
         class_name[len] = '\0';
      }
      else
      {
   ...
   else if (class_val.v.tag == TAG_TEMP_STRING)
   {
      string_node* s = GetTempString();
      if (s)
      {
         len = s->len_data < sizeof(class_name) - 1 ? s->len_data : sizeof(class_name) - 1;
         memcpy(class_name, s->data, len);
   ...
   {
      bprintf("C_GetItemTemplateByClass: class_name must be a string or resource\n");
      return ret_val.int_val;
   }

   it = GetItemTemplateByClass(class_name);

   if (it != NULL)
   {
      ret_val.v.data = it->item_template_id;
   }
...
/**
 * @brief KOD callable function: Checks if an item is active
 *
 * Parameter: #item_id (Integer) - Item Template ID
 *
 * @return TRUE if active, FALSE otherwise
 */
int C_IsItemActive(int object_id, local_var_type *local_vars,
                   int num_normal_parms, parm_node normal_parm_array[],
                   int num_name_parms, parm_node name_parm_array[])
{
   val_type item_id_val, ret_val;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_IsItemActive requires exactly 1 parameter (item_id)\n");
      return ret_val.int_val;
   }
   ...
   if (item_id_val.v.tag != TAG_INT)
   {
      bprintf("C_IsItemActive: item_id must be an integer\n");
      return ret_val.int_val;
   }

   item_id = item_id_val.v.data;
   ret_val.v.data = IsItemActive(item_id) ? True : False;

   return ret_val.int_val;
}

/**
 * @brief KOD callable function: Gets items by category
 *
 * Parameter: #category (Integer) - Item category (0=weapon, 1=armor, etc.)
 *
 * @return List of item template IDs or NIL if none found
 */
int C_GetItemsByCategory(int object_id, local_var_type *local_vars,
                         int num_normal_parms, parm_node normal_parm_array[],
                         int num_name_parms, parm_node name_parm_array[])
{
   val_type category_val, ret_val;
   int category;
   int results[MAX_LOCALS];
   int count;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_GetItemsByCategory requires exactly 1 parameter (category)\n");
      return ret_val.int_val;
   }

   category_val = RetrieveValue(object_id, local_vars, normal_parm_array[0].type,
                                 normal_parm_array[0].value);

   if (category_val.v.tag != TAG_INT)
   {
      bprintf("C_GetItemsByCategory: category must be an integer\n");
      return ret_val.int_val;
   }

   category = category_val.v.data;

   // Validate category range
   if (category < 0 || category > ITEM_CAT_MISC)
   {
      bprintf("C_GetItemsByCategory: invalid category %d\n", category);
      return ret_val.int_val;
   }

   count = GetItemsByCategory((item_category_t)category, results, MAX_LOCALS);
   ...
   val_type nil_val, list_val, item_val;
   ...
   {
      item_val.v.tag = TAG_INT;
      item_val.v.data = results[i];
      list_val.int_val = Cons(item_val, list_val);
   }
...
/* ============================================================================
 * PHASE 3: DB -> KOD GENERATION FUNCTIONS
 * ============================================================================
 */

/**
 * @brief KOD callable function: Creates an item object from a database template
 *
 * Parameter: #template_id (Integer) - Item template ID from item_templates table
 *
 * This function:
 * 1. Looks up the item template by ID
 * 2. Gets the KOD class name from item_kod_class field
 * 3. Creates a new object of that class
 * 4. Returns the new object
 *
 * @return Object ID of the created item, or NIL if creation failed
 */
int C_CreateItemFromTemplate(int object_id, local_var_type *local_vars,
                              int num_normal_parms, parm_node normal_parm_array[],
                              int num_name_parms, parm_node name_parm_array[])
{
   val_type template_val, ret_val;
   int template_id;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_CreateItemFromTemplate requires exactly 1 parameter (template_id)\n");
      return ret_val.int_val;
   }

   template_val = RetrieveValue(object_id, local_vars, normal_parm_array[0].type,
                                 normal_parm_array[0].value);

   if (template_val.v.tag != TAG_INT)
   {
      bprintf("C_CreateItemFromTemplate: template_id must be an integer\n");
      return ret_val.int_val;
   }

   template_id = template_val.v.data;

   /* Look up the template */
   item_template = GetItemTemplateByID(template_id);
   if (item_template == NULL)
   {
      bprintf("C_CreateItemFromTemplate: template %d not found\n", template_id);
      return ret_val.int_val;
   }
   ...
   {
      bprintf("C_CreateItemFromTemplate: template %d is not active\n", template_id);
      return ret_val.int_val;
   }

   /* Get the KOD class */
   if (item_template->item_kod_class[0] == '\0')
   {
      bprintf("C_CreateItemFromTemplate: template %d has no kod_class\n", template_id);
      return ret_val.int_val;
   }

   c = GetClassByName(item_template->item_kod_class);
   if (c == NULL)
   {
      bprintf("C_CreateItemFromTemplate: class '%s' not found for template %d\n",
              item_template->item_kod_class, template_id);
      return ret_val.int_val;
   }

   /* Create the object - pass named parameters through if any */
   new_object_id = CreateObject(c->class_id, num_name_parms, name_parm_array);

   if (new_object_id == INVALID_OBJECT)
   {
      bprintf("C_CreateItemFromTemplate: failed to create object for class '%s'\n",
              item_template->item_kod_class);
      return ret_val.int_val;
   }

   ret_val.v.tag = TAG_OBJECT;
   ret_val.v.data = new_object_id;

   dprintf("C_CreateItemFromTemplate: created object %d from template %d (%s)\n",
           new_object_id, template_id, item_template->item_kod_class);
...
/**
 * @brief KOD callable function: Creates a monster object from a database template
 *
 * Parameter: #template_id (Integer) - Monster template ID from monster_templates table
 *
 * This function:
 * 1. Looks up the monster template by ID
 * 2. Gets the KOD class name from monster_kod_class field
 * 3. Creates a new object of that class
 * 4. Returns the new object
 *
 * @return Object ID of the created monster, or NIL if creation failed
 */
int C_CreateMonsterFromTemplate(int object_id, local_var_type *local_vars,
                                 int num_normal_parms, parm_node normal_parm_array[],
                                 int num_name_parms, parm_node name_parm_array[])
{
   val_type template_val, ret_val;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_CreateMonsterFromTemplate requires exactly 1 parameter (template_id)\n");
      return ret_val.int_val;
   }
   ...
   if (template_val.v.tag != TAG_INT)
   {
      bprintf("C_CreateMonsterFromTemplate: template_id must be an integer\n");
      return ret_val.int_val;
   }
   ...
   /* Look up the template */
   monster_template = GetMonsterTemplateByID(template_id);
   if (monster_template == NULL)
   {
      bprintf("C_CreateMonsterFromTemplate: template %d not found\n", template_id);
      return ret_val.int_val;
   }

   /* Check if monster is active */
   if (!monster_template->active)
   {
      bprintf("C_CreateMonsterFromTemplate: template %d is not active\n", template_id);
      return ret_val.int_val;
   }

   /* Get the KOD class */
   if (monster_template->monster_kod_class[0] == '\0')
   {
      bprintf("C_CreateMonsterFromTemplate: template %d has no kod_class\n", template_id);
      return ret_val.int_val;
   }

   c = GetClassByName(monster_template->monster_kod_class);
   if (c == NULL)
   {
      bprintf("C_CreateMonsterFromTemplate: class '%s' not found for template %d\n",
              monster_template->monster_kod_class, template_id);
      return ret_val.int_val;
   }
   ...
   if (new_object_id == INVALID_OBJECT)
   {
      bprintf("C_CreateMonsterFromTemplate: failed to create object for class '%s'\n",
              monster_template->monster_kod_class);
      return ret_val.int_val;
   }
   ...
   dprintf("C_CreateMonsterFromTemplate: created object %d from template %d (%s)\n",
           new_object_id, template_id, monster_template->monster_kod_class);
...
/**
 * @brief KOD callable function: Gets the template ID for a monster class
 *
 * Parameter: #class_name (String or Resource) - KOD class name of the monster
 *
 * @return Integer template ID, or NIL if not found
 */
int C_GetMonsterTemplateID(int object_id, local_var_type *local_vars,
                            int num_normal_parms, parm_node normal_parm_array[],
                            int num_name_parms, parm_node name_parm_array[])
{
   val_type class_val, ret_val;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_GetMonsterTemplateID requires exactly 1 parameter (class_name)\n");
      return ret_val.int_val;
   }
   ...
   if (!LookupString(class_val, "C_GetMonsterTemplateID", &str, &len))
   {
      return ret_val.int_val;
   }

   /* Look up the template by class name */
   monster_template = GetMonsterTemplateByClass(str);
   if (monster_template == NULL)
   {
      return ret_val.int_val;
   }
   ...
   ret_val.v.tag = TAG_INT;
   ret_val.v.data = monster_template->monster_template_id;
...
/**
 * @brief KOD callable function: Gets monster templates by level range
 *
 * Parameters:
 *   #min_level (Integer) - Minimum level
 *   #max_level (Integer) - Maximum level
 *
 * @return List of template IDs matching the level range, or NIL if none found
 */
int C_GetMonsterTemplateByLevel(int object_id, local_var_type *local_vars,
                                 int num_normal_parms, parm_node normal_parm_array[],
                                 int num_name_parms, parm_node name_parm_array[])
{
   val_type min_val, max_val, ret_val;
   ...
   if (num_normal_parms != 2)
   {
      bprintf("C_GetMonsterTemplateByLevel requires 2 parameters (min_level, max_level)\n");
      return ret_val.int_val;
   }

   min_val = RetrieveValue(object_id, local_vars, normal_parm_array[0].type,
                            normal_parm_array[0].value);
   max_val = RetrieveValue(object_id, local_vars, normal_parm_array[1].type,
                            normal_parm_array[1].value);

   if (min_val.v.tag != TAG_INT || max_val.v.tag != TAG_INT)
   {
      bprintf("C_GetMonsterTemplateByLevel: parameters must be integers\n");
      return ret_val.int_val;
   }

   min_level = min_val.v.data;
   max_level = max_val.v.data;

   count = GetMonstersByLevel(min_level, max_level, results, MAX_LOCALS);
...
/**
 * @brief KOD callable function: Gets weapon-specific template data
 *
 * Parameter: #template_id (Integer) - Item template ID (must be a weapon)
 *
 * Returns a list: [weapon_type, weapon_quality, proficiency_needed, weapon_range, attack_type]
 * weapon_type: 0=thrust, 1=slash, 2=bludgeon
 * weapon_quality: 0=low, 1=normal, 2=high, 3=nerudite
 *
 * @return List of weapon data, or NIL if not found
 */
int C_GetWeaponTemplateData(int object_id, local_var_type *local_vars,
                             int num_normal_parms, parm_node normal_parm_array[],
                             int num_name_parms, parm_node name_parm_array[])
{
   val_type template_val, ret_val;
   int template_id;
   item_weapon_t *weapon_data;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_GetWeaponTemplateData requires exactly 1 parameter (template_id)\n");
      return ret_val.int_val;
   }
   ...
   if (template_val.v.tag != TAG_INT)
   {
      bprintf("C_GetWeaponTemplateData: template_id must be an integer\n");
      return ret_val.int_val;
   }
   ...
   /* Look up weapon data */
   weapon_data = GetItemWeaponData(template_id);
   if (weapon_data == NULL)
   {
      return ret_val.int_val;
   }

   /* Build return list: [weapon_type, weapon_quality, proficiency, range, attack_type] */
   val_type wtype, wquality, wprof, wrange, wattack;
   val_type nil_val, list_val;
   ...
   wtype.v.tag = TAG_INT;
   wtype.v.data = (int)weapon_data->weapon_type;

   wquality.v.tag = TAG_INT;
   wquality.v.data = (int)weapon_data->weapon_quality;

   wprof.v.tag = TAG_INT;
   wprof.v.data = weapon_data->proficiency_needed;

   wrange.v.tag = TAG_INT;
   wrange.v.data = weapon_data->weapon_range;

   wattack.v.tag = TAG_INT;
   wattack.v.data = weapon_data->attack_type;

   /* Build list from back to front */
   list_val.int_val = Cons(wattack, nil_val);
   list_val.int_val = Cons(wrange, list_val);
   list_val.int_val = Cons(wprof, list_val);
   list_val.int_val = Cons(wquality, list_val);
   list_val.int_val = Cons(wtype, list_val);
...
/**
 * @brief KOD callable function: Gets monster template data
 *
 * Parameter: #template_id (Integer) - Monster template ID
 *
 * Returns a list: [level, difficulty, max_hp, karma, is_undead, speed, vision, behavior]
 *
 * @return List of monster data, or NIL if not found
 */
int C_GetMonsterTemplateData(int object_id, local_var_type *local_vars,
                              int num_normal_parms, parm_node normal_parm_array[],
                              int num_name_parms, parm_node name_parm_array[])
{
   val_type template_val, ret_val;
   int template_id;
   monster_template_t *monster;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_GetMonsterTemplateData requires exactly 1 parameter (template_id)\n");
      return ret_val.int_val;
   }
   ...
   if (template_val.v.tag != TAG_INT)
   {
      bprintf("C_GetMonsterTemplateData: template_id must be an integer\n");
      return ret_val.int_val;
   }

   template_id = template_val.v.data;

   /* Look up monster template */
   monster = GetMonsterTemplateByID(template_id);
   if (monster == NULL)
   {
      return ret_val.int_val;
   }
   ...
   /* Build return list */
   val_type vlevel, vdiff, vhp, vkarma, vundead, vspeed, vvision, vbehavior;
   val_type nil_val, list_val;

   nil_val.v.tag = TAG_NIL;
   nil_val.v.data = NIL;

   vlevel.v.tag = TAG_INT;
   vlevel.v.data = monster->level;

   vdiff.v.tag = TAG_INT;
   vdiff.v.data = monster->difficulty;

   vhp.v.tag = TAG_INT;
   vhp.v.data = monster->max_hit_points;

   vkarma.v.tag = TAG_INT;
   vkarma.v.data = monster->karma;

   vundead.v.tag = TAG_INT;
   vundead.v.data = monster->is_undead;

   vspeed.v.tag = TAG_INT;
   vspeed.v.data = monster->speed;

   vvision.v.tag = TAG_INT;
   vvision.v.data = monster->vision_distance;

   vbehavior.v.tag = TAG_INT;
   vbehavior.v.data = monster->default_behavior;

   /* Build list from back to front */
   list_val.int_val = Cons(vbehavior, nil_val);
   list_val.int_val = Cons(vvision, list_val);
   list_val.int_val = Cons(vspeed, list_val);
   list_val.int_val = Cons(vundead, list_val);
   list_val.int_val = Cons(vkarma, list_val);
   list_val.int_val = Cons(vhp, list_val);
   list_val.int_val = Cons(vdiff, list_val);
   list_val.int_val = Cons(vlevel, list_val);
...
/**
 * @brief KOD callable function: Gets armor/defense template data
 *
 * Parameter: #template_id (Integer) - Item template ID
 *
 * Returns list: [defense_base, damage_base, spell_modifier, layer]
 * Returns $ if template not found or has no defense data
 */
int C_GetArmorTemplateData(int object_id, local_var_type *local_vars,
                            int num_normal_parms, parm_node normal_parm_array[],
                            int num_name_parms, parm_node name_parm_array[])
{
   val_type template_val, ret_val;
   int template_id;
   item_defense_t *defense_data;
   ...
   if (num_normal_parms != 1)
   {
      bprintf("C_GetArmorTemplateData requires exactly 1 parameter (template_id)\n");
      return ret_val.int_val;
   }
   ...
   if (template_val.v.tag != TAG_INT)
   {
      bprintf("C_GetArmorTemplateData: template_id must be an integer\n");
      return ret_val.int_val;
   }

   template_id = template_val.v.data;

   /* Look up defense data */
   defense_data = GetItemDefenseData(template_id);
   if (defense_data == NULL)
   {
      return ret_val.int_val;
   }

   /* Build return list: [defense_base, damage_base, spell_modifier, layer] */
   val_type vdefense, vdamage, vspell, vlayer;
   val_type nil_val, list_val;

   nil_val.v.tag = TAG_NIL;
   nil_val.v.data = NIL;

   vdefense.v.tag = TAG_INT;
   vdefense.v.data = defense_data->defense_base;

   vdamage.v.tag = TAG_INT;
   vdamage.v.data = defense_data->damage_base;

   vspell.v.tag = TAG_INT;
   vspell.v.data = defense_data->spell_modifier;

   vlayer.v.tag = TAG_INT;
   vlayer.v.data = defense_data->layer;

   /* Build list from back to front */
   list_val.int_val = Cons(vlayer, nil_val);
   list_val.int_val = Cons(vspell, list_val);
   list_val.int_val = Cons(vdamage, list_val);
   list_val.int_val = Cons(vdefense, list_val);
