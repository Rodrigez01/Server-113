...
void AdminExportItems(int session_id,admin_parm_type parms[],
                      int num_blak_parm,parm_node blak_parm[]);
void AdminExportQuests(int session_id,admin_parm_type parms[],
                       int num_blak_parm,parm_node blak_parm[]);
void AdminExportMonsters(int session_id,admin_parm_type parms[],
                         int num_blak_parm,parm_node blak_parm[]);
void AdminExportNPCs(int session_id,admin_parm_type parms[],
                     int num_blak_parm,parm_node blak_parm[]);
void AdminExportSpells(int session_id,admin_parm_type parms[],
                       int num_blak_parm,parm_node blak_parm[]);
void AdminExportSkills(int session_id,admin_parm_type parms[],
                       int num_blak_parm,parm_node blak_parm[]);
void AdminExportRooms(int session_id,admin_parm_type parms[],
                      int num_blak_parm,parm_node blak_parm[]);
...
admin_table_type admin_export_table[] =
{
	{ AdminExportItems,    {N}, F, A|M, NULL, 0, "items",    "Export item data from KOD to MySQL database" },
	{ AdminExportQuests,   {N}, F, A|M, NULL, 0, "quests",   "Export quest data from KOD to MySQL database" },
	{ AdminExportMonsters, {N}, F, A|M, NULL, 0, "monsters", "Export monster data from KOD to MySQL database" },
	{ AdminExportNPCs,     {N}, F, A|M, NULL, 0, "npcs",     "Export NPC data from KOD to MySQL database" },
	{ AdminExportSpells,   {N}, F, A|M, NULL, 0, "spells",   "Export spell data from KOD to MySQL database" },
	{ AdminExportSkills,   {N}, F, A|M, NULL, 0, "skills",   "Export skill data from KOD to MySQL database" },
	{ AdminExportRooms,    {N}, F, A|M, NULL, 0, "rooms",    "Export room data from KOD to MySQL database" },
};
#define LEN_ADMIN_EXPORT_TABLE (sizeof(admin_export_table)/sizeof(admin_table_type))
...
{ 
	...
	{ NULL, {N}, F, A|M, admin_export_table, LEN_ADMIN_EXPORT_TABLE, "export", "Export KOD data to MySQL (type 'help export' for options)" },
                      ...
                      int num_blak_parm,parm_node blak_parm[])
{
	...
	lprintf("AdminExportItems: Starting item export to MySQL database\n");
	aprintf("Exporting item data to MySQL database...\n");
	AdminSendBufferList();

	count = MySQLExportItemData();

	if (count > 0)
	{
		aprintf("Successfully exported %d items to database.\n", count);
		lprintf("AdminExportItems: Exported %d items\n", count);
	}
	else
	{
		aprintf("Export failed or no items found. Check server log for details.\n");
		lprintf("AdminExportItems: Export failed or returned 0 items\n");
	}
}
                         ...
                         int num_blak_parm,parm_node blak_parm[])
{
	...
	lprintf("AdminExportMonsters: Starting monster export to MySQL database\n");
	aprintf("Exporting monster data to MySQL database...\n");
	AdminSendBufferList();

	count = MySQLExportMonsterData();

	if (count > 0)
	{
		aprintf("Successfully exported %d monsters to database.\n", count);
		lprintf("AdminExportMonsters: Exported %d monsters\n", count);
	}
	else
	{
		aprintf("Export failed or no monsters found. Check server log for details.\n");
		lprintf("AdminExportMonsters: Export failed or returned 0 monsters\n");
	}
}
                     ...
                     int num_blak_parm,parm_node blak_parm[])
{
	...
	lprintf("AdminExportNPCs: Starting NPC export to MySQL database\n");
	aprintf("Exporting NPC data to MySQL database...\n");
	AdminSendBufferList();

	count = MySQLExportNPCData();

	if (count > 0)
	{
		aprintf("Successfully exported %d NPCs to database.\n", count);
		lprintf("AdminExportNPCs: Exported %d NPCs\n", count);
	}
	else
	{
		aprintf("Export failed or no NPCs found. Check server log for details.\n");
		lprintf("AdminExportNPCs: Export failed or returned 0 NPCs\n");
	}
}
                       ...
                       int num_blak_parm,parm_node blak_parm[])
{
	...
	lprintf("AdminExportSpells: Starting Spell export to MySQL database\n");
	aprintf("Exporting Spell data to MySQL database...\n");
	AdminSendBufferList();

	count = MySQLExportSpellData();

	if (count > 0)
	{
		aprintf("Successfully exported %d Spells to database.\n", count);
		lprintf("AdminExportSpells: Exported %d Spells\n", count);
	}
	else
	{
		aprintf("Export failed or no Spells found. Check server log for details.\n");
		lprintf("AdminExportSpells: Export failed or returned 0 Spells\n");
	}
}
                       ...
                       int num_blak_parm,parm_node blak_parm[])
{
	...
	lprintf("AdminExportSkills: Starting Skill export to MySQL database\n");
	aprintf("Exporting Skill data to MySQL database...\n");
	AdminSendBufferList();

	count = MySQLExportSkillData();

	if (count > 0)
	{
		aprintf("Successfully exported %d Skills to database.\n", count);
		lprintf("AdminExportSkills: Exported %d Skills\n", count);
	}
	else
	{
		aprintf("Export failed or no Skills found. Check server log for details.\n");
		lprintf("AdminExportSkills: Export failed or returned 0 Skills\n");
	}
}
                      ...
                      int num_blak_parm,parm_node blak_parm[])
{
	...
	lprintf("AdminExportRooms: Starting Room export to MySQL database\n");
	aprintf("Exporting Room data to MySQL database...\n");
	AdminSendBufferList();

	count = MySQLExportRoomData();

	if (count > 0)
	{
		aprintf("Successfully exported %d Rooms to database.\n", count);
		lprintf("AdminExportRooms: Exported %d Rooms\n", count);
	}
	else
	{
		aprintf("Export failed or no Rooms found. Check server log for details.\n");
		lprintf("AdminExportRooms: Export failed or returned 0 Rooms\n");
	}
}
                       ...
                       int num_blak_parm,parm_node blak_parm[])
{
	...
	lprintf("AdminExportQuests: Starting quest export to MySQL database\n");
	aprintf("Exporting quest data to MySQL database...\n");
	AdminSendBufferList();

	count = MySQLExportQuestData();

	if (count > 0)
	{
		aprintf("Successfully exported %d quests to database.\n", count);
		lprintf("AdminExportQuests: Exported %d quests\n", count);
	}
	else
	{
		aprintf("Export failed or no quests found. Check server log for details.\n");
		lprintf("AdminExportQuests: Export failed or returned 0 quests\n");
	}
}
